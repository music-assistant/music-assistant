...
class NugsProvider(MusicProvider):
    ...
    async def get_album_tracks(self, prov_album_id) -> list[Track]:
        ...
        return [
            self._parse_track(item, artist=artist, album=album, image=image)
    ...
    def _parse_track(self, track_obj: dict, artist=None, album=None, image=None) -> Track:
        ...
        if album:
            track.album = album
        if image is None and track_obj.get("image"):
            image = track_obj["image"]["url"]
        if image:
